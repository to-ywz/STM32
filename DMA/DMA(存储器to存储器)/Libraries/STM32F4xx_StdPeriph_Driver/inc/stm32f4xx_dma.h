...
typedef struct
{
  ...
  uint32_t DMA_Memory0BaseAddr;    /*!< Specifies the memory 0 base address for DMAy Streamx. 
                                        ...
                                        not enabled. */
	//数据传输方向 (Data transfer direction) DMA_SxCR的 DIR 2位 （7~6）表示四种状态
  uint32_t DMA_DIR;                /*!< Specifies if the data will be transferred from memory to peripheral, 
                                        from memory to memory or from peripheral to memory.
                                        This parameter can be a value of @ref DMA_data_transfer_direction */
	//DMA 数据流 x 数据项数寄存器 (DMA_SxNDTR) 16位 控制DMA传输多少个数据
	//单位由流控制寄存器中的 MSIZE 和 PSIZE 相关 开始传输后变为只读
  uint32_t DMA_BufferSize;         /*!< Specifies the buffer size, in data unit, of the specified Stream. 
                                        The data unit is equal to the configuration set in DMA_PeripheralDataSize
                                        or DMA_MemoryDataSize members depending in the transfer direction. */
	//外设递增模式 (Peripheral increment mode) DMA_SxCR的 PINC 1位， 表示外设地址是否递增
	//控制外设地址（不能递增，ADC寄存器唯一，DAC中）
  uint32_t DMA_PeripheralInc;      /*!< Specifies whether the Peripheral address register should be incremented or not.
                                        This parameter can be a value of @ref DMA_peripheral_incremented_mode */
	//存储器递增模式 (Memory increment mode) DMA_SxCR的 MINC 1位， 表示存储器地址扩容控制，表示存储器大小是否固定
	//控制存储器可用大小（如果一次传输多次采样，建议开启）
  uint32_t DMA_MemoryInc;          /*!< Specifies whether the memory address register should be incremented or not.
                                        This parameter can be a value of @ref DMA_memory_incremented_mode */
	//外设递增模式 (Peripheral increment mode) DMA_SxCR的 PSIZE 1位，表示外设数据宽度（单位）
	//P->M(源和目标的数据单位配置一致)(不一致：封装)
	/**
	  *	@note 1、源 == 目标 直接传输
	  *		  2、源 <  目标 只占用目标位置的源的大小，其余废弃
	  *       3、源 >  目标 数据丢失，只能保留目标大小
	 **/
  uint32_t DMA_PeripheralDataSize; /*!< Specifies the Peripheral data width.
                                        This parameter can be a value of @ref DMA_peripheral_data_size */
	//存储器递增模式 (Memory increment mode)   DMA_SxCR的 MSIZE 1位，表示存储器数据宽度（单位）
	//字、半字、字节（8 | 16 | 32）
  uint32_t DMA_MemoryDataSize;     /*!< Specifies the Memory data width.
                                        This parameter can be a value of @ref DMA_memory_data_size */
	//循环模式 (Circular mode) DMA_SxCR的 CIRC 1位，DMA传输模式选择 表示传输次数(1次/循环传输)、
	/**
	  * @note M->M 模式只能单次传输，即DMA_Mode_Normal模式
	**/
  uint32_t DMA_Mode;               /*!< Specifies the operation mode of the DMAy Streamx.
                                        This parameter can be a value of @ref DMA_circular_normal_mode
                                        @note The circular buffer mode cannot be used if the memory-to-memory
                                              data transfer is configured on the selected Stream */
	//优先级 (Priority level) DMA_SxCR的 PL 2位 表示四种优先级
